...
async def generate_cv(template_type: str = 'classic'):
    ...
    with open(f'app/generate_cv/templates/{template_type}/cv_data.json') as f: data = json.load(f)
    env = Environment(loader=FileSystemLoader(f'app/generate_cv/templates/{template_type}'))
    ...
    with open('app/generate_cv/cv_output.html', 'w') as f: f.write(html_out) # Save the rendered HTML
